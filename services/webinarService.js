...
function mapPublishedWebinar(webinar) {
  return {
    ...webinar,
    embedUrl: webinar.streamPlaybackId ? `https://iframe.videodelivery.net/${webinar.streamPlaybackId}` : '',
  };
}

async function listPublishedWebinars(WebinarModel) {
  const webinars = await WebinarModel.find({
    published: true,
    publishedAt: { $lte: new Date() },
  }).sort({ sortOrder: -1, publishedAt: -1, createdAt: -1 }).lean();

  return webinars.map(mapPublishedWebinar);
}
